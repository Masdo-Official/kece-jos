...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    setupAudio(el.audio, el.tombolMusik);
    setupTrigger(el.trigger, el.modal, el.input);
    setupModal(el.modal, el.input, el.errorMsg, el.tutupBtn);
    setupFormSubmit(el);
    setupMusikBtn(el.tombolMusik, el.audio);
  ...
  function queryAll() {
    const ids = {
      ...
      input: "kode-input",
      errorMsg: "error-msg",
      tutupBtn: "tutup-modal",
      pintu: "pintu-masuk",
      utama: "halaman-utama",
      pesanType: "pesan-typewriter",
      pesanDua: "pesan-dua",
      tombolMusik: "tombol-musik",
      audio: "audio-bg",
      foto: "foto-utama",
    };
    const out = {};
  ...
  /* ── AUDIO ────────────────────────────────────────────── */
  function setupAudio(audio, tombol) {
    if (!audio) return;
    audio.addEventListener("error", function () {
      tombol.classList.add("tersembunyi");
    });
  }
  ...
  /* ── MODAL ────────────────────────────────────────────── */
  function setupModal(modal, input, errorMsg, tutupBtn) {
    tutupBtn.addEventListener("click", function () {
      tutupModal(modal, input, errorMsg);
    });
    modal.addEventListener("click", function (e) {
      if (e.target === modal) tutupModal(modal, input, errorMsg);
    });
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && !modal.classList.contains("tersembunyi"))
        tutupModal(modal, input, errorMsg);
    });
  ...
  function terbukaModal(modal, input) {
  ...
  function tutupModal(modal, input, errorMsg) {
    modal.classList.add("tersembunyi");
    modal.setAttribute("aria-hidden", "true");
    input.value = "";
    if (errorMsg) {
      errorMsg.textContent = "";
      errorMsg.classList.add("tersembunyi");
    }
  }
  ...
  function setupFormSubmit(el) {
    ...
    function handle() {
      if (
        input.value.trim().toUpperCase() === CONFIG.kodeRahasia.toUpperCase()
      ) {
        tutupModal(modal, input, errorMsg);
        bukaHalamanUtama(
          pintu,
        ...
        errorMsg.classList.remove("tersembunyi");
  ...
  /* ── MUSIK ────────────────────────────────────────────── */
  function setupMusikBtn(tombol, audio) {
    if (!tombol || !audio) return;
    tombol.addEventListener("click", function () {
      if (audio.paused) {
        audio
          ...
          .then(function () {
            sedangBermain = true;
            updateBtn(tombol, true);
          })
          .catch(function () {
            sedangBermain = false;
            updateBtn(tombol, false);
          });
      } else {
        audio.pause();
        sedangBermain = false;
  ...
  function updateBtn(tombol, sedangBermain) {
    ...
    tombol.setAttribute(
      "aria-label",
      sedangBermain ? "Pause musik" : "Play musik",
    );
  }
  ...
  function bukaHalamanUtama(
    ...
    if (audio) {
      audio
        ...
        .then(function () {
          sedangBermain = true;
          tombolMusik.classList.remove("tersembunyi");
          updateBtn(tombolMusik, true);
        })
        .catch(function () {
          tombolMusik.classList.remove("tersembunyi");
          updateBtn(tombolMusik, false);
        });
    }
    ...
    setTimeout(function () {
      ...
      utama.classList.remove("tersembunyi");
  ...
  function initTilt(kartu) {
    ...
    (function loop() {
      cX += (tX - cX) * 0.07;
      cY += (tY - cY) * 0.07;
      kartu.style.transform = `perspective(1000px) rotateX(${cX.toFixed(3)}deg) rotateY(${cY.toFixed(3)}deg)`;
